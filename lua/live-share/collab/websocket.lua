...
local function sha1(msg)
  ...
  for i = 1, #msg, 64 do
    ...
    for j = 1, 16 do
      local o = i + (j - 1) * 4
      w[j] =
        bor(bor(bor(lshift(msg:byte(o), 24), lshift(msg:byte(o + 1), 16)), lshift(msg:byte(o + 2), 8)), msg:byte(o + 3))
    end
    for j = 17, 80 do
  ...
  for i = 1, 5 do
    local n = h[i]
    res[#res + 1] =
      string.char(band(rshift(n, 24), 0xFF), band(rshift(n, 16), 0xFF), band(rshift(n, 8), 0xFF), band(n, 0xFF))
  end
...
function M.encode_frame(payload, masked)
  ...
  elseif len < 65536 then
    header = string.char(0x82, b2base + 126, math.floor(len / 256), len % 256)
  else
    header = string.char(
      0x82,
      b2base + 127,
  ...
  local mk = { math.random(0, 255), math.random(0, 255), math.random(0, 255), math.random(0, 255) }
  local bytes = {}
  for i = 1, len do
  ...
  return function(data)
    ...
    while true do
      ...
      elseif plen7 == 126 then
        plen = buf:byte(3) * 256 + buf:byte(4)
      else
